    ...
    <v-layout>
      <v-flex xs12 sm6 offset-sm3>
        <v-card class='search-dish-card'>
          ...
          <v-layout row>
            <v-flex xs10><v-slider class='padded-slider' v-model='searchDishRating' :max='5' :min='0'></v-slider></v-flex>
            <v-flex xs1><v-text-field label='Rating' v-model='searchDishRating'></v-text-field></v-flex>
            <v-flex xs1><v-text-field prefix='stars' v-model='searchDishRating[1]'></v-text-field></v-flex>
          </v-layout>
...
export default {
  ...
  methods: {
    async searchDish () {
      let response = await DishesService.getUser({
        name: this.searchDishName,
        id: this.searchDishId,
        location: this.seachDishLocation,
        dietaryRestrictions: this.searchDishDietaryRestrictions,
        rating: this.searchDishRating
      })
